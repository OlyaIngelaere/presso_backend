...
<div class="card mt-5">
  <h2 class="card-header">Edit Answer - Capsule</h2>
  <div class="card-body">
    ...
    <form action="{{ route('answerCapsules.update',$answerCapsule->id) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="mb-3">
            ...
            <select 
                ...
                <?php foreach ($answers as $answer): ?>
                    <option value="<?= $answer->id ?>" <?= $answer->id === $answerCapsule->answerId ? 'selected' : '' ?> >
        ...
        <div class="mb-3">
            ...
            <select 
                ...
                <?php foreach ($capsules as $capsule): ?>
                    <option value="<?= $capsule->id ?>" <?= $capsule->id === $answerCapsule->capsuleId ? 'selected' : '' ?> >
                        <?= $capsule->name ?>
                    </option>
                <?php endforeach; ?>
            </select>
